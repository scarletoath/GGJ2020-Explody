...
public static class SpriteExploder
{
    ...
    public static List<GameObject> GenerateVoronoiPieces(GameObject source, int extraPoints = 0, int subshatterSteps = 0, Material mat = null, bool meshSaved = false)
    {
		...
		// Fix from https://forum.unity.com/threads/free-open-source-unity-2d-destruction.382416/#post-5012903
		Vector2 centerPivotDiff = Vector2.zero;
		SpriteRenderer sr = source.GetComponent<SpriteRenderer> ();
		if ( sr != null )
		{
			Vector2 srCenter = sr.bounds.center;
			Vector2 pivot = source.transform.position;
			centerPivotDiff = srCenter - pivot;
		}
		rect.center += centerPivotDiff;

		for (int i = 0; i < extraPoints; i++)
        {
			points.Add ( new Vector2 ( Random.Range ( rect.width / -2 , rect.width / 2 ) , Random.Range ( rect.height / -2 , rect.height / 2 ) ) + centerPivotDiff );
		}
